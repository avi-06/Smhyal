...
with st.sidebar:
  ...
  st.markdown("""The Smhyal algorithm is based on a modified version of an artificial neural network (ANN), with accuracies greater than 94% for  anxiety and depression diagnosis. The model was trained on the demographic and stressor data of 380,000+ young adults from the publicly-available Substance Abuse and Mental Health Service Administration (SAMHSA) Mental Health Client Level Data (MH-CLD) survey set. The app was created by Avi Verma, based on research work in collaboration with Stanford University. We hope that Smyhal will help postively impact young adults by counteracting the underdiagnosis of youth anxiety and depression.""")
